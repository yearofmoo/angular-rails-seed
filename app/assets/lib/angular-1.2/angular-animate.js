/**
 * @license AngularJS v1.1.6-2430347
  ...
  .config(['$provide', '$animateProvider', function($provide, $animateProvider) {
    ...
    $provide.decorator('$animate', ['$delegate', '$injector', '$sniffer', '$rootElement', '$timeout',
      ...
      return {
        ...
        */
        enter : function(element, parent, after, done, index) {
          $delegate.enter(element, parent, after);
          performAnimation('enter', 'ng-enter', element, parent, after, done, index);
        },
         ...
         * @param {function()=} done callback function that will be called once the animation is complete
        */
        leave : function(element, done, index) {
         ...
         * @param {function()=} done callback function that will be called once the animation is complete
        */
        move : function(element, parent, after, done, index) {
          $delegate.move(element, parent, after);
          performAnimation('move', 'ng-move', element, null, null, done, index);
        },
         ...
         * @param {function()=} done callback function that will be called once the animation is complete
        */
        addClass : function(element, className, done, index) {
         ...
         * @param {function()=} done callback function that will be called once the animation is complete
        */
        removeClass : function(element, className, done, index) {
      ...
      */
      function performAnimation(event, className, element, parent, after, onComplete, index) {
        ...
        forEach(animations, function(animation, i) {
          var fn = function() {
            progress(i);
          };
          ...
          if(animation.start) {
            if(event == 'addClass' || event == 'removeClass') {
              animation.endFn = animation.start(element, baseClassName, fn, index);
            } else {
              animation.endFn = animation.start(element, fn, index);
            }
          } else {
            fn();
          }
    ...
    $animateProvider.register('', ['$window','$sniffer', '$timeout', function($window, $sniffer, $timeout) {
      var noop = angular.noop;
      var forEach = angular.forEach;
      function animate(element, className, done, index) {
        if (!($sniffer.transitions || $sniffer.animations)) {
          done();
        } else {
          var activeClassName = '';
          element.addClass('ng-stagger');
          $timeout(function() {
            var staggerDelay = parseStaggerDelay(index);
            element.removeClass('ng-stagger');
            $timeout(startAnimation, staggerDelay, false);
          }, 1, false);
        ...
        function parseStaggerDelay(index) {
          ...
          return Math.max(transitionDelay, animationDelay) * 1000 * index + 1;
        }
      ...
      return {
        enter : function(element, done, index) {
          return animate(element, 'ng-enter', done, index);
        },
        leave : function(element, done, index) {
          return animate(element, 'ng-leave', done, index);
        },
        move : function(element, done) {
          return animate(element, 'ng-move', done, index);
        },
        addClass : function(element, className, done, index) {
          return animate(element, suffixClasses(className, '-add'), done, index);
        },
        removeClass : function(element, className, done, index) {
          return animate(element, suffixClasses(className, '-remove'), done, index);
        }
      };
